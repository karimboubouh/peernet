...
class LogisticRegression(object):
    ...
    def __init__(self, lr=4, epochs=9, batch_size=128, beta=0.9, threshold=0.5, debug=True, name=None, seed=0):
        np.random.seed(seed)
        self.lr = lr
        self.epochs = epochs
    ...
    def test_accuracy(self, X_test, Y_test):
        ...
        test_accuracy = 1.0 - np.mean(np.abs(Y_prediction_test - Y_test) * 1.0)
    ...
    def loss(cls, Y, A):
        ...
        if m == 0:
            return None
        cost = -1.0 / m * np.sum(Y * np.log(A) + (1.0 - Y) * np.log(1.0 - A + 1e-7))
